...
class SmartRecordController:
    ...
    def _log_sr_properties(self, elem_ptr, name, source_id):
        ...
        import ctypes
        gobj_lib = ctypes.CDLL("libgobject-2.0.so.0")

        def _get_uint(ptr, prop):
            val = ctypes.c_uint(0)
            gobj_lib.g_object_get(
                ctypes.c_void_p(ptr), prop.encode(),
                ctypes.byref(val), ctypes.c_void_p(0),
            )
            return val.value

        def _get_str(ptr, prop):
            val = ctypes.c_char_p()
            gobj_lib.g_object_get(
                ctypes.c_void_p(ptr), prop.encode(),
                ctypes.byref(val), ctypes.c_void_p(0),
        ...
        logger.info(
            ...
            _get_uint(elem_ptr, "smart-record"),
            _get_str(elem_ptr, "smart-rec-dir-path"),
            _get_str(elem_ptr, "smart-rec-file-prefix"),
            _get_uint(elem_ptr, "smart-rec-cache"),
            _get_uint(elem_ptr, "smart-rec-default-duration"),
        )
    ...
    def _wait_for_playing(self, elem_ptr, source_id, timeout=15):
        ...
        import ctypes
        gst_lib = ctypes.CDLL("libgstreamer-1.0.so.0")
        gst_lib.gst_element_get_state.argtypes = [
            ctypes.c_void_p, ctypes.POINTER(ctypes.c_int),
            ctypes.POINTER(ctypes.c_int), ctypes.c_uint64,
        ]
        gst_lib.gst_element_get_state.restype = ctypes.c_int
        ...
        for attempt in range(timeout * 2):
            gst_lib.gst_element_get_state(
                ctypes.c_void_p(elem_ptr),
                ctypes.byref(state), None,
                ctypes.c_uint64(ONE_SEC_NS // 2),
            )
            if state.value == GST_STATE_PLAYING:
                return True
            time.sleep(0.5)
