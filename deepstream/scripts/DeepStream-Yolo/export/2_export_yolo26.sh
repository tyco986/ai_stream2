...
Export YOLO26 .pt to ONNX + labels.txt for DeepStream (container: ultralytics_export).

Required:
  -w PATH              Host path to weights (.pt)

Optional (passed to export_yolo26.py):
...
Prerequisites: 0_pull_base_image.sh, 1_build_container.sh
EOF
}
...
docker exec -w "$C" ultralytics_export python3 /app/DeepStream-Yolo-master/utils/export_yolo26.py -w "${C}/${F}" --opset 18 "$@"
