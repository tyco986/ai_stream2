...
from utils.peoplenet.detectnet_e2e import DetectNetE2EGraft
...
class PeopleNetExporter:
    ...
    def validate_backbone(self, inputs: list[dict], outputs: list[dict]) -> None:
        if len(inputs) != 1:
            raise ValueError(f"peoplenet expected one graph input, got {len(inputs)}")
        input_t = inputs[0]
        if input_t["name"] != PEOPLENET_INPUT_NAME:
            raise ValueError(
                f"peoplenet expected input {PEOPLENET_INPUT_NAME!r}, got {input_t['name']!r}"
            )
        chw = tuple(input_t["dims"][1:4])
        if chw != PEOPLENET_INPUT_CHW:
            raise ValueError(
                f"peoplenet expected input CHW {PEOPLENET_INPUT_CHW}, got {chw}"
            )
        output_names = tuple(tensor["name"] for tensor in outputs)
        if set(output_names) != set(PEOPLENET_OUTPUT_NAMES):
            raise ValueError(
                f"peoplenet expected outputs {PEOPLENET_OUTPUT_NAMES}, got {output_names}"
            )
    ...
    def build_meta(self, inputs: list[dict], outputs: list[dict]) -> dict:
        ...
        return {
            ...
            "classes": list(PEOPLENET_CLASSES),
    ...
    def export(
        ...
        grafted = DetectNetE2EGraft(
            model, PEOPLENET_NMS_CONF, PEOPLENET_NMS_IOU, PEOPLENET_MAX_DET
        ).graft()
