...
from utils.api.services import AppError, ExportEngineService
...
async def lifespan(app: FastAPI):
    app.state.export_engine = ExportEngineService(MODEL_ROOT)
    (MODEL_ROOT / "onnx").mkdir(parents=True, exist_ok=True)
    (MODEL_ROOT / "trt").mkdir(parents=True, exist_ok=True)
    logger = logging.getLogger(LOGGER_NAME)
    logger.info("Export TRT API started log_root=%s model_root=%s", LOG_ROOT, MODEL_ROOT)
    yield
    logger.info("Export TRT API stopped")
...
def create_app() -> FastAPI:
    configure_logging()
    app = FastAPI(title="Export TRT API", version="1.0.0", lifespan=lifespan)
