...
DEFAULT_WEIGHT="${ROOT}/models/pt/yolo26n.pt"

usage() {
  cat <<EOF
usage: $0 [-w path/to/model.pt] [export options...]

Export YOLO26 .pt to ONNX + labels.txt for DeepStream (container: DeepStream-Yolo).

Optional:
  -w PATH              Host path to weights (.pt); default: models/pt/yolo26n.pt

Other flags (passed to export_yolo26.py):
...
docker exec -w "$C" DeepStream-Yolo python3 /app/DeepStream-Yolo-master/utils/export_yolo26.py -w "${C}/${F}" --opset 18 "$@"
