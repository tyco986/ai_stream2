...
TOPDOWN_POSE_VIS_RTSP_TOPOLOGY_DOC = """
    Topology::
        ...
        nvurisrcbin{N} → nvstreammux → pgie → nvbboxsnapshot → nvtracker → nvdspreprocess_rtmpose → sgie0
            → nvrtmposepostprocess → nvdsanalytics → nvstreamdemux
              → queue_demux{N} → nvvideoconvert{N} → nvposefadedrawer{N} → nvosdbin{N}
