...
TOPDOWN_POSE_SAHI_RTSP_TOPOLOGY_DOC = """
    Topology::

        nvurisrcbin{N} → nvstreammux → nvsahipreprocess → pgie → queue_sahi → nvsahipostprocess
              → nvtracker → sgie0 → nvdsanalytics → nvstreamdemux
...
class TopdownPoseSahiRTSPGenerator(TopdownPoseMixin, BaseSahiRTSPGenerator):
    GENERATOR = "TopdownPoseSahiRTSPGenerator"
    SINK_PATH_TEMPLATES = {
        "fakesink{index}": [
            "nvurisrcbin{index}",
            "nvstreammux",
    ...
    f"""Generate topdown-pose SAHI RTSP pipeline (headless).
