...
class PgieConfigParser:
    ...
    def build(self) -> dict:
        ...
        yolo_version = self.meta.get("yolo_version")
        assert yolo_version in self.SUPPORTED_YOLO_VERSIONS, (
            f"unsupported yolo_version in {self.META_JSON_NAME}: {yolo_version}"
        )
        ...
        if self.class_on is not None:
            ...
            assert attr_class_ids <= set(self.class_on), (
                f"class_attr class ids {sorted(attr_class_ids - set(self.class_on))} "
                f"not in class_on"
            )

        if any("iou" in attrs for attrs in self.class_attr.values()):
            assert yolo_version in ("yolo8", "yolo11"), (
                f"iou not supported for {yolo_version}"
            )
        class_attrs = {
        ...
        class_on_set = set(self.class_on) if self.class_on is not None else None
        filter_out_class_ids = (
            ";".join(
                str(class_id)
                for class_id in range(num_classes)
                if class_id not in class_on_set
            )
            if self.class_on is not None
            else None
        )
