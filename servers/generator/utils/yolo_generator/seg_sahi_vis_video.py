import copy

from ..base_generator.base_sahi_vis_video import (
    BaseSahiVisVideoGenerator,
    SAHI_VIS_VIDEO_TOPOLOGY_DOC,
)
from ..subelement_generator.utils.default_gie import YoloSeg


class SegSahiVisVideoGenerator(BaseSahiVisVideoGenerator):
    GENERATOR = "SegSahiVisVideoGenerator"

    f"""Generate YOLO SAHI segmentation video pipeline YAML with OSD and mp4 filesink.

    {SAHI_VIS_VIDEO_TOPOLOGY_DOC}
    """

    def apply_pgie_config(self) -> None:
        self.pgie_generator.config = copy.deepcopy(YoloSeg)
        self.pgie_generator.update_config()
        self.pgie_yml = self.pgie_generator.config

    def osd_kwargs(self, gpu_id: int) -> dict:
        return {
            "gpu_id": gpu_id,
            "display_bbox": True,
            "display_text": True,
            "display_mask": True,
        }
