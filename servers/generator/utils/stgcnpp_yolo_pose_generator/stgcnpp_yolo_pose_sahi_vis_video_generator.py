...
from ..base_generator.base_sahi_pose_vis_video import BaseSahiPoseVisVideoGenerator
from ..subelement_generator.nvtracker import TRACKER_LL_LIB
from .stgcnpp_yolo_pose_mixin import StgcnppYoloPoseMixin

STGCNPP_YOLO_POSE_SAHI_VIS_VIDEO_TOPOLOGY_DOC = """
    Topology::

        nvurisrcbin → nvstreammux → nvsahipreprocess → pgie → queue_sahi → nvsahipostprocess_pose
            → nvbboxsnapshot → nvtracker → nvdspreprocess → sgie1 → nvdsanalytics → nvosdbin
...
class StgcnppYoloPoseSahiVisVideoGenerator(StgcnppYoloPoseMixin, BaseSahiPoseVisVideoGenerator):
    GENERATOR = "StgcnppYoloPoseSahiVisVideoGenerator"
