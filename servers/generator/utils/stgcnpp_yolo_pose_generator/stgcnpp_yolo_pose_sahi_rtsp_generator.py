from ..base_generator.base_sahi_pose_rtsp import BaseSahiPoseRTSPGenerator
from ..subelement_generator.nvtracker import TRACKER_LL_LIB
from .stgcnpp_yolo_pose_mixin import StgcnppYoloPoseMixin

STGCNPP_YOLO_POSE_SAHI_RTSP_TOPOLOGY_DOC = """
    Topology::

        nvurisrcbin{N} → nvstreammux → nvsahipreprocess → pgie → queue_sahi → nvsahipostprocess_pose
              → nvbboxsnapshot → nvtracker → nvdspreprocess → sgie1 → nvdsanalytics → nvstreamdemux
...
class StgcnppYoloPoseSahiRTSPGenerator(StgcnppYoloPoseMixin, BaseSahiPoseRTSPGenerator):
    GENERATOR = "StgcnppYoloPoseSahiRTSPGenerator"
