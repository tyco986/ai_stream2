...
STGCNPP_RTMPOSE_VIS_RTSP_TOPOLOGY_DOC = """
    Topology::

        nvurisrcbin{N} → nvstreammux → pgie → nvbboxsnapshot → nvtracker → nvdspreprocess_rtmpose → sgie0 → nvdspreprocess
...
class StgcnppRtmposeVisRTSPGenerator(StgcnppRtmposeMixin, BaseRTSPVisGenerator):
    GENERATOR = "StgcnppRtmposeVisRTSPGenerator"

    f"""Generate ST-GCN++ RTSP pipeline with OSD preview.
