...
STGCNPP_RTMPOSE_SAHI_VIS_VIDEO_TOPOLOGY_DOC = """
    Topology::

        nvurisrcbin → nvstreammux → nvsahipreprocess → pgie → queue_sahi → nvsahipostprocess
            → nvbboxsnapshot → nvtracker → nvdspreprocess_rtmpose → sgie0 → nvdspreprocess → sgie1 → nvdsanalytics → nvosdbin
...
class StgcnppRtmposeSahiVisVideoGenerator(StgcnppRtmposeMixin, BaseSahiVisVideoGenerator):
    GENERATOR = "StgcnppRtmposeSahiVisVideoGenerator"

    f"""Generate ST-GCN++ SAHI vis-video pipeline YAML with OSD and mp4 filesink.
