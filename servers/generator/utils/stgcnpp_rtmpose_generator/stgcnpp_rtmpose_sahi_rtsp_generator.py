from ..base_generator.base_sahi_rtsp import BaseSahiRTSPGenerator
from ..subelement_generator.nvtracker import TRACKER_LL_LIB
from .stgcnpp_rtmpose_mixin import StgcnppRtmposeMixin

STGCNPP_RTMPOSE_SAHI_RTSP_TOPOLOGY_DOC = """
    Topology::

        nvurisrcbin{N} → nvstreammux → nvsahipreprocess → pgie → queue_sahi → nvsahipostprocess
              → nvbboxsnapshot → nvtracker → nvdspreprocess_rtmpose → sgie0 → nvdspreprocess → sgie1 → nvdsanalytics → nvstreamdemux
...
class StgcnppRtmposeSahiRTSPGenerator(StgcnppRtmposeMixin, BaseSahiRTSPGenerator):
    GENERATOR = "StgcnppRtmposeSahiRTSPGenerator"

    f"""Generate ST-GCN++ SAHI RTSP pipeline (headless, ends at fakesink).
    ...
    def link(self) -> None:
        ...
        self.link_stgcnpp(edges, "nvrtmposepostprocess")
