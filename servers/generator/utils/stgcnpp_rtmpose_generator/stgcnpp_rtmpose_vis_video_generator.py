...
STGCNPP_RTMPOSE_VIS_VIDEO_TOPOLOGY_DOC = """
    Topology::

        nvurisrcbin → nvstreammux → pgie → nvbboxsnapshot → nvtracker → nvdspreprocess_rtmpose → sgie0 → nvdspreprocess → sgie1
            → nvdsanalytics → nvosdbin → nvvideoconvert → nvv4l2h264enc → h264parse
            → mp4mux → filesink
"""


class StgcnppRtmposeVisVideoGenerator(StgcnppRtmposeMixin, BaseVisVideoGenerator):
    GENERATOR = "StgcnppRtmposeVisVideoGenerator"
