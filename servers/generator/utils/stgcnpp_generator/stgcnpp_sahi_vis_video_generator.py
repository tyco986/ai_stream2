...
STGCNPP_SAHI_VIS_VIDEO_TOPOLOGY_DOC = """
    Topology::

        nvurisrcbin → nvstreammux → nvsahipreprocess → pgie → queue_sahi → nvsahipostprocess
            → nvtracker → sgie0 → nvdspreprocess → sgie1 → nvdsanalytics → nvosdbin
...
class StgcnppSahiVisVideoGenerator(StgcnppMixin, BaseSahiVisVideoGenerator):
    GENERATOR = "StgcnppSahiVisVideoGenerator"
    SINK_PATH_TEMPLATES = {
        "filesink": [
            "nvurisrcbin",
            "nvstreammux",
