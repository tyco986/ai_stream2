/**
 * Streams API client (contract-aligned mock).
 * Paths: /{PROJECT_NAME}/backend/streams/...
    ...
    streams: [
      {
        ...
        enabled: true,
        recording: false,
        last_test_at: '2026-06-27T18:22:00Z',
      },
      {
      ...
      },
      {
        ...
        enabled: true,
        recording: false,
        last_test_at: '2026-06-28T09:40:00Z',
      },
      {
...
export async function patchStream(
  streamId: string,
  body: Partial<
    Pick<Stream, 'name' | 'group_id' | 'url' | 'enabled' | 'recording'>
  >,
): Promise<Stream> {
  const store = readStore()
...
export async function testStream(streamId: string): Promise<TestResult> {
  const store = readStore()
  const stream = store.streams.find((item) => item.id === streamId)
  if (!stream) {
    throw new ApiError(404, 'Stream not found')
...
export async function batchDelete(body: { stream_ids: string[] }): Promise<void> {
  const store = readStore()
  const remove = new Set(body.stream_ids)
  store.streams = store.streams.filter((item) => !remove.has(item.id))
  writeStore(store)
}

export async function batchEnable(body: { stream_ids: string[] }): Promise<void> {
  const store = readStore()
  const ids = new Set(body.stream_ids)
  for (const stream of store.streams) {
...
export async function batchDisable(body: { stream_ids: string[] }): Promise<void> {
  const store = readStore()
  const ids = new Set(body.stream_ids)
  for (const stream of store.streams) {
    if (ids.has(stream.id)) {
...
export async function batchTest(body: {
  ...
  for (const id of body.stream_ids) {
    results.push(await testStream(id))
  }
  return { results }
}
