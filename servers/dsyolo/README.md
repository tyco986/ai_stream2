...
在 `ai_stream2_dsyolo` 镜像中运行的 FastAPI 服务，将 `.pt` 权重导出为 DeepStream 可用的 ONNX 产物。

## 构建与运行

```bash
# 项目根目录
./servers/dsyolo/scripts/0_pull_base_image.sh   # 可选
./servers/dsyolo/scripts/1_build_image.sh
./servers/dsyolo/scripts/2_run_container.sh
```

容器名 `DeepStream-Yolo`，端口 `8090`。挂载 `models` → `/root/models`，`logs` → `/root/logs`。

API：`http://127.0.0.1:8090/ai_stream2/dsyolo/hello_world`  
Swagger：`http://127.0.0.1:8090/docs`

## 导出流程

1. `input` 上传 `.pt` 文件，落盘到 `/root/models/pt/{name}.pt`（同名直接覆盖）
2. 清空并创建 `/root/models/onnx/{name}/`，以 `pt/` 下 weights 为 `-w`、`onnx/{name}/` 为 cwd 调用 DeepStream 导出脚本（ONNX 生成在 `pt/` 同目录，`labels.txt` 在 cwd）
...
```bash
curl -s -X POST http://127.0.0.1:8090/ai_stream2/dsyolo/export_yolo26 \
...
```bash
./servers/dsyolo/scripts/3_export_yolo26.sh --input models/pt/yolo26n.pt
```
...
| `HOST` / `--host` | `0.0.0.0` |
| `PORT` / `--port` | `8090` |
| `LOG_ROOT` / `--log-root` | `/root/logs/dsyolo` |
