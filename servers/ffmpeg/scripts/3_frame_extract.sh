...
Extract one frame via FFmpeg API. PNG is written to outputs/ffmpeg/frame_extract/.
...
HTTP_CODE="$(curl -sS -w "%{http_code}" -o /tmp/ffmpeg_frame_extract_response.json \
  -X POST "${ENDPOINT}" \
  -F "input=${CONTAINER_INPUT}" \
  -F "timestamp=${TIMESTAMP}")"

cat /tmp/ffmpeg_frame_extract_response.json
echo
