...
`POST /schema` 响应 `data`。对应 UI **Pipeline** / **Generator** 两组。
...
| `generator.params` | object | 固定含 `streams` / `pgie` / `interval` / `tracker` / `analyzer`（全量，禁止缺键） |

约定：

| 规则 | 说明 |
|------|------|
| 全量 | 每个 Type 的响应必须带齐上述全部区块与叶子字段；用 `available: false` 表示不可用，**不要省略键** |
| `available` | 决定 UI 是否可编辑；与 [pipeline.md](./pipeline.md) Type 规则一致 |
| `default` | 选中 Type 后写入表单的初始值；切换 Type 时用新 schema 覆盖 |
