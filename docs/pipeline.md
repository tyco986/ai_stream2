...
| 区域 | 说明 |
|------|------|
| **Type** | 下拉；选项来自 `GET /types`；选定后调用 `POST /schema` 拉取全量表单 schema；见 [Type](#type) |
...
选定（或切换）某个 Type 后，前端立即调用 **`POST /pipelines/schema`**，入参为该 `pipeline_type`；响应为该 Type 下 **Pipeline** / **Generator** 两侧的**全量**表单 schema（含每个区块/字段的 `available` 与 `default`，**禁止缺省字段**）。前端据此渲染控件的可编辑态与默认值，**不再**在页面内用 Type 名硬编码「是否显示 Debouncer / Streams」等规则。
