...
> 灰色仅表示 **Disabled**；**Offline 且仍 Enabled** 的流保持黑色，与表格 **Status** 列（Online / Offline / Disabled）语义一致。
...
| FPS | `int` | 只读 |
| Status | `str` | 只读；展示：`enabled=false` → **`Disabled`**；否则 `online`→**`Online`**、`offline`→**`Offline`**（API 字段 `status` 与 `enabled` 分离） |
...
| 行内 / 工具栏 **Test** 完成 | — | 结果写入表格 **Status** / **Last Test** 等；失败格内红色，**无 Toast** |
