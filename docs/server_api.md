...
后端静态或配置文件维护，**无** CRUD API。`id` 与 `name` 均为 `servers/` 下文件夹名（同值）；另须配置 `container_name`、`health_upstream`（实际健康检测地址或探测逻辑）；对外一律代理为 `hello_world`。默认条目：
