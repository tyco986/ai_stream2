...
每条服务带 **`category`**：`infrastructure`（静态微服务）或 `pipeline`（各 Pipeline 对应 DeepStream 容器，`id` 形如 `pipeline:{uuid}`）。静态表**不再**包含共享 `deepstream`。
...
| 分组 | **Infrastructure** 与 **Pipeline** 两组；每组一张只读表 |
...
| 列名 | 类型 | 说明 |
|------|------|------|
| Name | `str` | Infrastructure：文件夹名；Pipeline：pipeline **name**；只读 |
| Status | `str` | 只读；`Online` / `Offline`；由最近一次健康探测结果决定 |
| Last Refresh | `datetime` | 只读；该服务**最近一次探测完成**时间；格式 `MM-DD HH:mm:ss`（如 `06-30 14:22:05`）；从未探测时为 `—` |
