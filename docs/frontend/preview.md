...
宫格槽位以**红色框线**（**1px**）标出边界；槽间**共用**该 1px（交界不叠成 2px），画面仍紧挨填满网格。选中槽叠加高亮叠层，框线始终可见。
...
| **Groups · Clear 🧹** | 与 Preset 栏 **Clear** 相同；清空全部槽位并取消选中槽 |
...
| 点击左侧流 | 未绑定则加入宫格；已绑（`*`）见槽位规则（无选中/同槽解绑，异槽换绑） |
| 当前选中槽 | 选中高亮叠层；槽位**红色 1px 框线**始终可见；新点流优先填入该格，无选中格则填下一空槽 |
...
| 点击**流 Name** | 未绑定：按槽位规则加入宫格；已绑（`*`）：无选中或同槽 → 解绑；有选中且异槽 → 换绑到选中槽 |
| 展开 / 折叠组 | 仅影响树展示 |
...
| 行为 | `slots` 全 `null`；清除当前选中槽；**不改** `layout` / `view_mode` |
| 持久化 | **不**调 API，直至 **Save**；Preset 下拉名标 `*` |
| 可用条件 | 至少 1 槽已绑定，**或**当前有选中槽时可点；无绑定且无选中时 **disabled** |
...
行末 `*` 表示该流已占用某一宫格槽位。再点：无选中槽或选中槽即该流所在槽 → 解绑；选中槽为其他槽 → 换绑到选中槽。
...
| 项 | 说明 |
|----|------|
| 行为 | 清空当前布局全部已绑定流（`slots` 全 `null`）；清除当前选中槽；**不改** `layout` |
...
| 视觉 | 与 **Grid** 宫格一致（含 **1px** 红色槽位框线）；**不含** Focus 侧栏、Preset 栏、滚动条 |
...
| **Focus 图标 ◎** | 当前为 Grid 时点击 → 进入 Focus Mode；**有选中槽**则以该槽为焦点，**无选中槽**则以槽 `0` 为焦点（空槽亦可） |
| 高亮 | 当前 `view_mode` 对应图标高亮；另一图标默认色 |
| 持久化 | `view_mode`（`grid` \| `focus`）随 **Save** / **Save As** 写入预设；切换图标未 **Save** 时 **Preset** 标 `*` |
| disabled | **Focus 图标始终可点**（不依赖选中态、不依赖槽是否已绑流） |
...
| 场景 | 行为 |
|------|------|
| 点击左侧**未绑定**流，**无选中槽** | 填入序号最小的空槽 |
| 点击左侧**未绑定**流，**有选中槽** | 替换选中槽绑定 |
| 点击左侧**已绑定**流（`*`），**无选中槽** | 取消该流绑定（对应槽置 `null`） |
| 点击左侧**已绑定**流（`*`），**有选中槽且为该流所在槽** | 取消该流绑定 |
| 点击左侧**已绑定**流（`*`），**有选中槽且为其他槽** | 换绑到选中槽（原槽置 `null`，选中槽写入该流） |
| 点击空槽 | 设为当前选中槽 |
| 点击已占用槽 | 设为当前选中槽（不停止播放） |
| 槽位已满且无选中槽 | 点击左侧**未绑定**流 → **替换最后一个槽**（序号 `N−1`）为新流；被替换槽停止播放；当前选中槽设为最后槽 |
| **Clear** | 清空全部已绑定流（`slots` 全 `null`）；清除当前选中槽；**不改** `layout`；**不**自动保存；有变动时 **Preset 栏** 标 `*` |

### 单格结构

宫格内各槽以**红色 1px 框线**分隔，画面区域**紧挨**填满网格（槽间无额外间隙）；信息层（Name、状态、底栏）叠于画面上或槽内顶/底。
...
| 元素 | 说明 |
|------|------|
| 槽位框线 | 红色、**1px**；各槽始终显示；交界处共用 1px（非每格自绘后相加）；选中时叠加高亮，框线不隐藏 |
...
Grid（任意选中态 / 槽是否绑流均可）
        │
        ▼
Preset 栏点 Focus ◎（始终可点）
        │
        ├─ 有选中槽 → focus_index = 选中槽下标
        └─ 无选中槽 → focus_index = 0
        │
        ▼
...
| 操作 | 说明 |
|------|------|
| 点 **Focus ◎** | 进入 Focus Mode；有选中槽则以该槽为焦点，否则焦点为槽 `0`（空槽亦可） |
| 点 **Grid ⊞** | 退出 Focus Mode，恢复 Grid |
| 点击右侧列表项 | 切换焦点槽（含空槽）；主区同步；列表高亮跟随 |
| 点击左侧树中流 | 未绑定：按槽位规则写入 `slots` 并设焦点；已绑（`*`）：同槽/无选中解绑，异槽换绑 |
| 主区格内 **×** | 清空该槽绑定（`null`）；焦点切到列表中下一已绑定槽；若全无绑定则主区显示空占位，**仍保持 Focus** |
| **Clear** | `slots` 全 `null`；清除选中槽；**保持 Focus**；右侧列表保留 layout 等量空占位（如 2×2 仍 4 行） |
...
| **Clear** | 扫把图标按钮；紧挨 **Save As** | 清空全部槽位流绑定并清除选中槽；Focus 下**保持** Focus 与列表占位 |
...
左侧点击未绑流      ──> 绑定 stream；满槽无选中时替换最后槽；未 Save 则 Preset 下拉标 *
左侧点击已绑流（*） ──> 无选中或同槽：解绑；有选中且异槽：换绑到选中槽
点击空槽            ──> 设为当前选中槽
点击已占用槽        ──> 设为当前选中槽
点 Focus ◎          ──> view_mode = focus；未 Save 则 Preset 标 *
点 Grid ⊞           ──> view_mode = grid；未 Save 则 Preset 标 *
Focus 下点列表项    ──> 切换焦点槽（含空槽）
Focus 下左侧点流    ──> 未绑：按槽位规则绑定并设焦点；已绑（*）：同槽/无选中解绑，异槽换绑
Focus 下 Layout 下拉 ──> 列表行数随规格变；slots 保留/截断；标 *
Clear 图标          ──> 清空 slots 并清除选中槽（Preset 栏或 Groups 栏 🧹）；不改 layout；未 Save 则标 *；Focus 下保持 Focus 与列表占位
左侧搜索            ──> 过滤树节点 Name
...
左侧 Groups 🧹      ──> 同 Preset Clear：清空 slots 并清除选中槽；未 Save 则标 *
...
| 同流多槽 | 同一 **stream id** 同时最多占 1 个槽；再点已绑流（`*`）：无选中/同槽解绑，异槽换绑到选中槽 |
...
| Clear | 清空当前布局全部流并清除选中槽；**不**改 `layout`；**不**写入后端，直至 **Save** |
